...
`TokenDataStore` wraps a `DataStore<AuthPreferences>` with an `AeadSerializer` from `androidx.datastore.tink`. The actual encryption key is an AES256-GCM keyset managed by `AndroidKeysetManager`, with the keyset itself wrapped by a key that never leaves the **Android Keystore** (hardware backed on most devices).
...
| Encryption | Google Tink (`tink-android` 1.21.0), AES256-GCM, Android Keystore backed |
